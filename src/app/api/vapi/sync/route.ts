...
const TOOL_DEFS: Record<string, { description: string; properties: Record<string, unknown>; required: string[] }> = {
  ...
  // The handler was already in functions/route.ts but had no synced tool,
  // so the agent could check availability but never commit a booking.
  create_booking: {
...
export async function POST() {
  ...
  // Session 17B -- create_booking added so Growth/Pro agents can actually
  // commit a booking after check_availability returns a free slot.
