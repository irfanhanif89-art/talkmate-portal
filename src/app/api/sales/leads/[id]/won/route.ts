...
import { COMMISSION_MAP, COMMISSION_POLICY_VERSION, isCommissionPlan } from '@/lib/commission'
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const commissionAmount = COMMISSION_MAP[plan]
  ...
  // Create the pending commission row. Amount is server-side from
  ...
  await admin.from('lead_activities').insert([
    ...
    },
    {
      ...
      body: `Pending commission: $${commissionAmount}`,
    },
  ])
