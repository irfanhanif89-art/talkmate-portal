...
export async function POST(request: NextRequest) {
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      // Session generated by /api/sales/leads/[id]/payment-link carries
      // client_reference_id = lead.id, so we can credit the closing rep's
      // commission with no email-matching guesswork. Runs BEFORE the
      // business-side lookup (this is a lead, not a business — the
      // business shell hasn't been created yet; admin will do that via
      // /api/admin/onboarding-queue/create-from-lead).
      if (session.client_reference_id) {
        ...
        if (paidLead) {
          await supabase
            ...
            .eq('id', paidLead.id)
          // Approve the pending commission for this lead (any rep).
          const { data: approvedCommissions } = await supabase
            .from('commissions')
            .update({ status: 'approved', approved_at: new Date().toISOString() })
            .eq('lead_id', paidLead.id)
            .eq('status', 'pending')
            .select('id, rep_id')
          for (const comm of approvedCommissions ?? []) {
            await supabase.from('rep_notifications').insert({
              ...
              lead_id: paidLead.id,
              message: `Payment confirmed for ${paidLead.business_name}. Your commission is now approved.`,
            })
          }
          await supabase.from('lead_activities').insert({
            ...
            body: `Customer paid via Stripe Checkout (session ${session.id}). Commission approved.`,
          })
          await sendAdminTelegram(
            `Payment received: ${paidLead.business_name} (${approvedCommissions?.length ?? 0} commission row(s) approved).`,
          ).catch(() => {})
        }
      }
