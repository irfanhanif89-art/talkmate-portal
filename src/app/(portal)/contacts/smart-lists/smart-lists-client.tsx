  ...
  return (
    <div style={{ padding: 28, color: '#F2F6FB' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 32, marginBottom: 12 }}>
        <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>Custom lists</h2>
        <button
          onClick={() => setBuilderOpen(true)}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--orange)', color: 'white', border: 'none', borderRadius: 9, padding: '9px 14px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
        >
          <Plus size={13} /> Create custom list
        </button>
      </div>
      {custom.length === 0 ? (
        <div style={{ background: 'var(--card)', border: '1px dashed rgba(255,255,255,0.08)', borderRadius: 14, padding: 32, textAlign: 'center', fontSize: 13, color: 'var(--dim)' }}>
...
function Section({ title, lists, hideHeading }: { title: string; lists: SmartList[]; hideHeading?: boolean }) {
  ...
  return (
    <div style={{ marginTop: hideHeading ? 0 : 0 }}>
      {!hideHeading && (
        <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>{title}</h2>
      )}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 12 }}>
        {lists.map(l => (
          <Link
            ...
            style={{
              background: 'var(--card)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: 18,
          ...
          >
            ...
            {l.description && (
              <div style={{ fontSize: 12, color: 'var(--dim)', marginTop: 4, lineHeight: 1.5, minHeight: 36 }}>{l.description}</div>
            )}
