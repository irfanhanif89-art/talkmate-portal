...
**Last session:** 36
**Main SHA:** 49b2a1b (Session 35 hotfix pending merge to main; Session 36 hotfix pending)
...
| SCORING_PROVIDER | optional | Call Intelligence provider switch (Session 36 hotfix). Values: `anthropic` (default — Claude Sonnet) or `xai` (Grok `grok-4.20-0309-non-reasoning` via existing `GROK_API_KEY`). Any other value falls back to `anthropic`. Set per-environment so production can run on Grok while preview/dev stays on Anthropic. |
