...
| 47 | 2026-05-27 | feature/profile-editing-and-show-password | 3f68f3f | — | Profile editing (admin + rep) + show-password toggle on login. New `PasswordInput` reusable component (eye-icon toggle for show/hide) wired into `src/app/(auth)/login/page.tsx`. New `EditRepModal` admin-side modal for editing any contractor/rep's profile fields. New `POST /api/admin/contractors/[id]/profile` admin route (311 lines) for the edit endpoint. Existing `POST /api/sales/profile` route expanded (~+150 lines) for rep self-edit of profile fields beyond just `notification_email`. `src/components/sales/profile-form.tsx` significantly expanded (+250/-19) to render the new editable surface. `src/app/(portal)/profile/page.tsx` and `src/app/sales/profile/page.tsx` updated to use the new form. `src/app/(portal)/admin/contractors/[id]/contractor-detail-view.tsx` wires the Edit button into the new modal. Audit logging hook added in `src/lib/audit.ts`. PR #47. No migration, no new env vars. (Session-log entry filled in retroactively by the talkmate-mobile Sales Rep session — only the visible commit + file diff was available to the documenting agent; if richer detail exists, expand from the build session's own notes.) |
